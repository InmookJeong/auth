package kr.mook.auth.terms.remove;
...
public class RemoveTermsTest {
	...
	@MockBean
	private RemoveTermsMapper _removeTermsMapper;
	...
	@Sql(
		statements = {
			...
			");" +
			"INSERT INTO TABLE_SEQUENCES VALUES('terms_no', 2);" + 
			"DELIMITER $$ " + 
	...
	@BeforeEach
	void createTempSequenceTableAndData() {}
	 ...
	 */
	@Test
	void testRemoveByUnvaildTermsNoWithLocaleKoKr() throws Exception {
		long termsNo = 0L;
		String httpStatusCode = "400";
		String statusCode = "ERR-TMS-DEL-001";
		String status = "DELETE ERROR";
		String resultMessage = "이용약관 번호가 올바르지 않습니다. 이용약관 번호는 1이상의 숫자를 입력해주세요.";
		String apiDocsDir = "terms/remove/remove-by-unvaild-terms-no/ko";
		ResultMatcher resultMatcher = status().isBadRequest();
		
		this._testRemoveResponse(termsNo, this._LOCALE_KO_KR, this._ACCEPT_LANGUAGE_KO_KR, httpStatusCode, statusCode, status, resultMessage, apiDocsDir, resultMatcher);
	}
	 ...
	 */
	@Test
	void testRemoveByUnvaildTermsNoWithLocaleEnUs() throws Exception {
		long termsNo = 0L;
		String httpStatusCode = "400";
		String statusCode = "ERR-TMS-DEL-001";
		String status = "DELETE ERROR";
		String resultMessage = "The Terms of Use number is incorrect. Please enter a number greater than 1.";
		String apiDocsDir = "terms/remove/remove-by-unvaild-terms-no/en";
		ResultMatcher resultMatcher = status().isBadRequest();
		
		this._testRemoveResponse(termsNo, this._LOCALE_EN_US, this._ACCEPT_LANGUAGE_EN_US, httpStatusCode, statusCode, status, resultMessage, apiDocsDir, resultMatcher);
	}
	
	/**
	 * 삭제할 이용약관 정보를 찾을 수 없는 경우<br/>
	 * - 이용약관 정보를 최종적으로 삭제하는 과정에서 데이터를 찾을 수 없는 경우, 이용약관 정보를 찾을 수 없다는 메시지가 출력되는지 테스트<br/>
	 * - 삭제할 이용약관 정보를 찾을 수 없는 경우 404에러 발생<br/>
	 * - 결과 메시지는 한글로 출력되도록 다국어 적용
	 * 
	 * @throws Exception
	 */
	@Test
	void testRemoveNotFoundErrorWithLocaleKoKr() throws Exception {
		long termsNo = 5L;
		
		// 이용약관 정보를 수정할 데이터가 없는 경우
		given(this._removeTermsMapper.delete(termsNo)).willReturn(0);
		
		String httpStatusCode = "404";
		String statusCode = "ERR-TMS-DEL-002";
		String status = "DELETE ERROR";
		String resultMessage = "삭제할 이용약관 정보가 없습니다. 이용약관 번호를 다시 확인해주세요.";
		String apiDocsDir = "terms/remove/terms-not-found/ko";
	 ...
	 */
	@Test
	void testRemoveNotFoundErrorWithLocaleEnUs() throws Exception {
		long termsNo = 5L;
		...
		String statusCode = "ERR-TMS-DEL-002";
		String status = "DELETE ERROR";
		String resultMessage = "There are no Terms of Use information to delete. Please check the Terms of Use number again.";
		String apiDocsDir = "terms/remove/terms-not-found/en";
		ResultMatcher resultMatcher = status().isNotFound();
	...
	/**
	 * 이용약관 정보 삭제에 성공하는 경우<br/>
	 * - 이용약관 정보를 최종적으로 삭제하는지 테스트<br/>
	 * - 정상적으로 삭제될 경우 저장 결과 메시지가 출력되는지 테스트<br/>
	 * - 이용약관 정보가 삭제될 경우 200 상태 코드 발생<br/>
	 * - 결과 메시지는 한글로 출력되도록 다국어 적용
	 * 
	 * @throws Exception
	 */
	@Test
	void testRemoveWithLocaleKoKr() throws Exception {
		long termsNo = 1L;
		
		// 이용약관 정보 삭제에 성공한 경우
		given(this._removeTermsMapper.delete(termsNo)).willReturn(1);
		
		String httpStatusCode = "200";
		String statusCode = "TMS-DEL-001";
		String status = "DELETE";
		String resultMessage = "이용약관 정보가 삭제되었습니다.";
		String apiDocsDir = "terms/remove/success/ko";
	 ...
	 */
	@Test
	void testRemoveWithLocaleEnUs() throws Exception {
		long termsNo = 1L;
		...
		String statusCode = "TMS-DEL-001";
		String status = "DELETE";
		String resultMessage = "The Terms of Use information has removed.";
		String apiDocsDir = "terms/remove/success/en";
		ResultMatcher resultMatcher = status().isOk();
	...
	/**
	 * 이용약관 번호 조회 테스트<br/>
	 * - 이용약관 정보 삭제 중 오류가 발생하였을 경우에 대한 테스트<br/>
	 * - 이용약관 정보 삭제 성공에 대한 테스트<br/>
	 * 
	 ...
	 * @throws Exception
	 */
	private void _testRemoveResponse(Long termsNo, Locale locale, String acceptLanguage, String httpStatusCode, String statusCode, String status, String resultMessage, String apiDocsDir, ResultMatcher resultMatcher) throws Exception {
		mockMvc.perform(delete("/api/terms/{termsNo}", termsNo)
